...
class ActionExecutor:
    ...
    def execute(self, action_dict):
        ...
        if action == "unlockDoor":
            ...
            self.mqtt.publish(build_topic(self.room_id, "room", "command"), cmd)
            ...
            self.mqtt.publish(build_topic(self.room_id, "game", "status"), cmd)
            if status == "completed":
                ended = SessionEndedEvent(room_id=self.room_id, duration=3600, success=True)
                self.mqtt.publish(build_topic(self.room_id, "session", "ended"), ended)
